...
def start_forward(ip):
    global forward_proc
    print(f"[{time.strftime('%F %T')}] forwarding localhost:{LOCAL_PORT} -> {ip}:{REMOTE_PORT}")
...
while True:
    ...
    # extract all sequences of digits+dots (handles concatenated IPs)
    for match in re.findall(rb'[\d\.]+', data):
        ip = match.decode()
        if ip != current_ip:
            if current_ip:
                stop_forward()
